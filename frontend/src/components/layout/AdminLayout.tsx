...
const menuItems: MenuItemType[] = [
    ...
    { text: 'Relatórios e BI', icon: <Assessment />, path: '/admin/relatorios' },
    { text: 'Ações', icon: <Event />, path: '/admin/acoes' },
    { text: 'Instituições', icon: <Business />, path: '/admin/instituicoes' },
    { text: 'Caminhões', icon: <LocalShipping />, path: '/admin/caminhoes' },
    { text: 'Funcionários', icon: <People />, path: '/admin/funcionarios' },
    { text: 'Cursos/Exames', icon: <School />, path: '/admin/cursos-exames' },
];
...
const AdminLayout: React.FC = () => {
            ...
            api.get('/admins/me')
                .then((response) => {
                    if (response.data.foto_perfil) {
                        setAdminFoto(`http://localhost:3001${response.data.foto_perfil}`);
                    }
                })
                .catch((error) => {
    ...
    const isActive = (path: string) => {
        // Para o dashboard, verificar se é exatamente /admin
        if (path === '/admin') {
